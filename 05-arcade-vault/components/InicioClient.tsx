...
export function InicioClient({ game, topScores }: { game: Game; topScores: ScoreRow[] }) {
  useReveal();
  const router = useRouter();

  const ticker = topScores.slice(0, 7);
  const topPlayers = topScores.slice(0, 5);
    ...
    <div className="home fade-in">
      ...
      <section className="home-section reveal">
        ...
        <div className="mini-rail">
          <MiniCard key={game.id} game={game} onClick={() => router.push(`/juego/${game.id}`)} />
        </div>
        ...
        <div className="activity-grid">
          <div className="activity-card">
            ...
            <div className="ticker">
              {ticker.length === 0 && (
                <div style={{ color: "var(--ink-faint)", padding: "16px 0" }}>Todavía no hay puntuaciones.</div>
              )}
              {ticker.map((row, i) => (
